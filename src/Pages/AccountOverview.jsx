          ...
          <div className="flex items-center justify-center text-center">
            <div>
              <h4 className="text-3xl font-bold mb-3 flex items-center justify-center">
                ...
                OXO MARKETS Account Types
              </h4>
              <p className="text-lg text-purple-100">Explore various account types to suit your trading needs.</p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full min-w-[800px]">
              <thead>
                <tr className="bg-gradient-to-r from-black via-gray-900 to-black text-white">
                  <th className="py-4 px-3 md:py-6 md:px-6 text-left font-bold text-lg md:text-xl border-r border-gray-700">Account Types</th>
                  <th className="py-4 px-3 md:py-6 md:px-6 text-center font-bold text-sm md:text-lg border-r border-gray-700">
                    <div className="flex flex-col md:flex-row items-center justify-center">
                      <svg className="w-5 h-5 md:w-6 md:h-6 md:mr-3 mb-1 md:mb-0" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <th className="py-4 px-3 md:py-6 md:px-6 text-center font-bold text-sm md:text-lg">
                    <div className="flex flex-col md:flex-row items-center justify-center">
                      <svg className="w-5 h-5 md:w-6 md:h-6 md:mr-3 mb-1 md:mb-0" fill="currentColor" viewBox="0 0 20 20">
              ...
              <tbody>
                {[
                  ...
                  <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-blue-50 transition-colors duration-200`}>
                    <td className="py-3 px-3 md:py-4 md:px-6 font-semibold text-gray-900 border-r border-gray-200 text-sm md:text-base">{row.label}</td>
                    {row.values.map((value, idx) => (
                      <td key={idx} className={`py-3 px-3 md:py-4 md:px-6 text-center text-gray-700 border-r border-gray-200 last:border-r-0 text-xs md:text-sm ${row.longText ? 'max-w-[120px] md:max-w-none' : ''}`}>
                        <span className={row.longText ? 'block truncate md:whitespace-normal md:text-left' : ''} title={row.longText ? value : ''}>
                          {value}
                        </span>
                      </td>
                    ))}
